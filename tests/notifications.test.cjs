...
function fixture(extra = {}) {
  const seed = {
    ...
    preferences: [
      ...
      {
        id: "customer",
        notificationPreferences: { email: true, emailEnabledAt: now - 1000 },
      },
      {
        id: "other",
        notificationPreferences: { email: true, emailEnabledAt: now - 1000 },
    ...
    notifications: [
      {
        ...
        message: "Order AW-1 was submitted for Shop One.",
        recordId: "order-one",
        createdAt: now,
      },
    ],
  ...
  seed.notifications = seed.notifications.map((row) => ({
    emailQueuePending: true,
    ...row,
  }));
...
test("missing sender configuration preserves queued work and performs no send", async () => {
  const repo = fixture();
  await queueNotifications(repo, { now });
...
test("delivery marks provider acceptance as sent with a stable Message-ID", async () => {
  ...
  await queueNotifications(repo, { now });
  const calls = [];
  const result = await deliverOutbox(repo, {
    transport: transport(calls),
    from: "Alabama Wholesale <orders@example.test>",
    now,
    id: ids(),
  });
...
test("atomic claims stop concurrent workers from sending the same message twice", async () => {
  ...
  await startedGate;
  const two = await deliverOutbox(repo, {
    transport: slow,
    from: "orders@example.test",
    now,
    id: () => "worker-two",
...
test("delivery rechecks opt-out, store access, current email and active status", async () => {
  for (const change of ["optout", "store", "email", "inactive"]) {
    ...
    await queueNotifications(repo, { now });
    if (change === "optout")
      await repo.put("preferences", "customer", {
        id: "customer",
        notificationPreferences: { email: false },
      });
    else
      await repo.put("users", "customer", {
        ...(await repo.get("users", "customer")),
        ...(change === "store"
          ? { storeIds: ["two"] }
          : change === "email"
...
test("a definite SMTP rejection is failed and is retried only by explicit administrator action", async () => {
  ...
  const rejected = {
    async sendMail() {
      calls++;
      throw Object.assign(new Error("SECRET credentials/server detail"), {
        responseCode: 550,
        code: "EENVELOPE",
        command: "RCPT TO",
      });
  ...
  await assert.rejects(
    () =>
      retryOutbox(repo, row.id, {
        actor: { uid: "customer", role: "customer" },
        reason: "Try again",
        now,
      }),
    (e) => e.status === 403,
  );
  await retryOutbox(repo, row.id, {
    actor: owner,
    reason: "Recipient server fixed",
    now,
  });
  const sent = [];
  assert.equal(
...
test("ambiguous failures and expired send leases are uncertain and never retried automatically", async () => {
  ...
  const ambiguous = {
    async sendMail() {
      calls++;
      throw Object.assign(new Error("Socket lost"), {
        code: "ESOCKET",
        command: "DATA",
      });
    },
  ...
  assert.equal(calls, 1);
  await assert.rejects(
    () => retryOutbox(repo, row.id, { actor: owner, reason: "Try again", now }),
    (e) => e.code === "duplicate_risk_acknowledgement_required",
  );
  await retryOutbox(repo, row.id, {
    actor: owner,
    reason: "Confirmed no receipt; retry requested",
    acknowledgeDuplicateRisk: true,
    now,
  });
  await repo.put("outbox", row.id, {
    ...(await repo.get("outbox", row.id)),
    status: "sending",
...
test("SMTP transport configuration enforces TLS without exposing credentials or opening a connection", () => {
  ...
  assert.equal(
    createSmtpTransport({
      smtpUrl: "smtp://user:secret@smtp.example.test:587",
  ...
  assert.throws(
    () => createSmtpTransport({ smtpUrl: "https://example.test/SECRET" }),
    (e) =>
      e.code === "invalid_smtp_configuration" && !e.message.includes("SECRET"),
  );
});

test("completed queue scans do not repeat recipient transactions, and interrupted scans resume", async () => {
  const repo = fixture();
  await repo.put("preferences", "owner", {
    id: "owner",
    notificationPreferences: { email: true, emailEnabledAt: now - 1 },
  });
  const original = repo.transaction.bind(repo);
  let calls = 0;
  repo.transaction = (fn) => {
    if (++calls === 2) throw new Error("simulated storage outage");
    return original(fn);
  };
  await assert.rejects(
    () => queueNotifications(repo, { now }),
  ...
  repo.transaction = () => {
    throw new Error("completed scan must not open another transaction");
  };
  assert.equal((await queueNotifications(repo, { now })).scanned, 0);
});
    ...
    original((tx) =>
      callback({
        ...
        set: async (collection, id, value) => {
          if (collection === "outbox" && value.status === "sent")
            throw new Error("storage unavailable after provider accepted");
          return tx.set(collection, id, value);
        },
      }),
    );
  const calls = [];
