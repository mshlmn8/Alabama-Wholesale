...
function select(records, options = {}) {
  ...
  if (options.startAfter) {
    const index = out.findIndex((r) => r.id === options.startAfter);
    if (index >= 0) out = out.slice(index + 1);
  }
  return out.slice(0, options.limit ?? out.length).map(clone);
}
...
class FirestoreRepository {
  ...
  async transaction(callback) {
    return this.db.runTransaction(async (firestoreTx) => {
      ...
      if (writes.size > 450)
        throw Object.assign(
          new Error("This operation is too large for one transaction."),
          { status: 413, code: "transaction_too_large" },
        );
      for (const value of writes.values()) {
