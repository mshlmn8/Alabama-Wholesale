...
function variantFor(product,variant) {
  ...
  if(variants.length?(!variants.includes(out)&&!(out===''&&product.standardVariantEnabled===true)):out!=='')fail('INVALID_VARIANT',`Choose a valid variant for ${product.name||'this product'}.`);
  return out;
}
function draftLines(lines,{allowEmpty=true}={}) {
  if(!Array.isArray(lines)||(!allowEmpty&&lines.length===0))fail('INVALID_INPUT','An order requires a list of lines and at least one line to submit.');
  const ids=new Set();
...
function priceFor(product,variant,store) {
  ...
  // Unpriced catalog items submit at zero; malformed prices still fail validation.
  return money(price??0,'Product price');
}
async function loadProductCategories(reader,products) {
  const pending=products.flatMap(product=>Array.isArray(product.categoryIds)?product.categoryIds:[]),seen=new Set(),categories=new Map();
  for(let index=0;index<pending.length;index++) {
    const id=pending[index];if(typeof id!=='string'||!id||seen.has(id))continue;seen.add(id);
    const category=await reader.get('categories',id);if(!category)continue;
    categories.set(id,category);if(category.parentId)pending.push(category.parentId);
  }
  return categories;
}
function productCategoryNames(product,categories) {
  const names=new Set();
  for(const id of Array.isArray(product.categoryIds)?product.categoryIds:[]) {
    const path=[],seen=new Set();let current=id;
    while(current&&!seen.has(current)) {
      seen.add(current);const category=categories.get(current);if(!category)break;
      if(typeof category.name==='string'&&category.name.trim())path.push(category.name.trim());
      current=category.parentId;
    }
    for(const name of path.reverse())names.add(name);
  }
  return [...names];
}
function calculateOrder(lines,products,store,categories=[]) {
  ...
  const categoryById=categories instanceof Map?categories:new Map(categories.map(category=>[category.id,category])),categoryNamesByProduct=new Map();
  ...
  const snapshots=normalized.map(line=>{
    ...
    if(!categoryNamesByProduct.has(product.id))categoryNamesByProduct.set(product.id,productCategoryNames(product,categoryById));
    return {...line,name:text(product.name,'Product name',300,true),categoryNames:categoryNamesByProduct.get(product.id),sku:text(product.sku||product.id,'SKU',200,true),barcode:text(product.variantBarcodes?.[variant]||product.barcode,'Barcode',200),variant,packSize,eachQuantity,eachPriceCents,unitPriceCents,lineTotalCents,taxable,taxRateBps:taxable?taxRateBps:0,taxCents};
  });
...
function imageUrl(value) {
  const raw=text(value,'Image',2000);if(!raw)return '';
  const out=raw.startsWith('images/')?'/'+raw:raw;
  if(/^\/media\/products\/[a-f0-9]{64}\.(png|jpg|webp)$/.test(out))return out;
  if(out.startsWith('/assets/')||out.startsWith('/images/')) {
    if(/[?#\\]/.test(out))fail('INVALID_INPUT','The image path is invalid.');
    const segments=out.split('/').slice(2);
    if(!segments.length||segments.some(segment=>!segment))fail('INVALID_INPUT','The image path is invalid.');
    for(const segment of segments) {
      let decoded;
      try{decoded=decodeURIComponent(segment);}catch{fail('INVALID_INPUT','The image path contains invalid encoding.');}
      for(let depth=0;depth<8;depth++) {
        if(decoded==='.'||decoded==='..'||/[\/\\\u0000-\u001F]/.test(decoded))fail('INVALID_INPUT','The image path cannot contain traversal or encoded separators.');
        if(!/%[a-f0-9]{2}/i.test(decoded))break;
        let next;try{next=decodeURIComponent(decoded);}catch{break;}
        if(next===decoded)break;
        if(depth===7)fail('INVALID_INPUT','The image path encoding is too deeply nested.');
        decoded=next;
      }
    }
    return out;
  ...
  fail('INVALID_INPUT','Use an approved local image or an HTTPS image URL.');
}
...
async function notify(tx,{storeId=null,userId=null,audience=null,type,message,recordId:targetId=null},context) {
  const id=context.id();
  await tx.set('notifications',id,{id,storeId,userId,audience:audience||(storeId?'store':userId?'user':'staff'),type,message,recordId:targetId,readBy:[],emailQueuePending:true,createdAt:context.now,version:1});
}
...
async function invoiceFinancials(tx,order) {
  const [payments,returns]=await Promise.all([tx.list('payments',{where:[['storeId','==',order.storeId]]}),tx.list('returns',{where:[['orderId','==',order.id]]})]);
  return {payments,returns};
}
async function refreshInvoiceAmounts(tx,order,context,financials=null) {
  const data=financials||await invoiceFinancials(tx,order);
  const figures=invoiceAmounts(order,data.payments,data.returns);
...
async function executeCommand(tx,actor,command,{now=Date.now(),id=randomUUID}={}) {
  ...
  if(command.type==='category.save') {
    ...
    const parentId=text(payload.parentId??previous?.parentId??'','Parent category',200);
    const seen=new Set([categoryId]);
    for(let ancestor=parentId;ancestor;) {
      if(seen.has(ancestor)||seen.size>1000)fail('INVALID_INPUT','A category cannot be placed inside itself or one of its subcategories.');
      seen.add(ancestor);
      const parent=await required(tx,'categories',recordId(ancestor),'Parent category');
      ancestor=parent.parentId||'';
    }
    result=versioned(previous,{id:categoryId,name:text(payload.name??previous?.name,'Category name',150,true),parentId,sortOrder:integer(payload.sortOrder??previous?.sortOrder??0,'Sort order',{max:100000}),active:bool(payload.active,'Active',previous?.active!==false)},now,actor);
    ...
    // Keep the original blank Standard choice valid after adding named variants.
    // Clients cannot grant or revoke this compatibility flag.
    result.standardVariantEnabled = previous?.standardVariantEnabled === true || !!(previous && !previous.variants?.length && variants.length);
    if (previous && result.image !== previous.image) result.imageSource = null;
    ...
    for(const line of lines){if(!products.has(line.productId))products.set(line.productId,await required(tx,'products',line.productId,'Product'));variantFor(products.get(line.productId),line.variant);}
    ...
    if(!previous) {
      // Allocate once in the same transaction as the draft and its receipt.
      // Existing orders and the separate fiscal invoice sequence stay unchanged.
      const counterId='order-numbers',counter=await tx.get('counters',counterId);
      const value=counter?integer(counter.value,'Order sequence',{max:Number.MAX_SAFE_INTEGER-1}):0;
      const version=counter?integer(counter.version,'Order sequence version',{max:Number.MAX_SAFE_INTEGER-1}):0;
      result.orderNumber=value+1;
      await tx.set('counters',counterId,{id:counterId,value:result.orderNumber,version:version+1});
    }
    ...
    if(previous.status!=='draft') {
      ...
      const categories=await loadProductCategories(tx,products);
      const calculated=calculateOrder(previous.lines,products,store,categories);
      ...
      const balance=storeBalance(await tx.list('ledger',{where:[['storeId','==',store.id]]}),store.id);
    ...
    if(status===previous.status)result=previous;
      ...
      if(status==='cancelled'&&previous.invoiceNumber){const data=await invoiceFinancials(tx,result);Object.assign(result,invoiceAmounts(result,data.payments,data.returns));}
      await tx.set('orders',previous.id,result);
    ...
    if(previous.status==='verified')result=previous;
      ...
      let allocations=[],allocationWarning=null,targetOrder=null,financials=null;
      if(previous.orderId){
        ...
        else {requireAllocatable(targetOrder);financials=await invoiceFinancials(tx,targetOrder);const figures=invoiceAmounts(targetOrder,financials.payments,financials.returns);const allocated=Math.min(previous.amountCents,figures.amountDueCents);if(allocated>0)allocations=[{orderId:targetOrder.id,amountCents:allocated}];}
      }
      ...
      if(targetOrder&&allocations.length)await refreshInvoiceAmounts(tx,targetOrder,context,{...financials,payments:financials.payments.map(payment=>payment.id===result.id?result:payment)});
    ...
    const [payments,returns]=await Promise.all([tx.list('payments',{where:[['storeId','==',store.id]]}),tx.list('returns',{where:[['storeId','==',store.id]]})]);const affected=new Map();
    ...
    const financials={payments:payments.map(payment=>payment.id===result.id?result:payment),returns};
    for(const order of affected.values())await refreshInvoiceAmounts(tx,order,context,financials);
    ...
    if(onHand-reserved<=result.reorderPoint)await notify(tx,{audience:'staff',type:'inventory.low',message:`Low stock: ${product.name}${variant?' — '+variant:''}.`,recordId:key},context);
    ...
    if(!Array.isArray(payload.lines)||payload.lines.length===0)fail('INVALID_INPUT','Choose at least one return line.');
    const existing=(await tx.list('returns',{where:[['orderId','==',order.id]]})).filter(r=>r.orderId===order.id&&['pending','approved'].includes(r.status));
    const seen=new Set();
    ...
    if(previous.status==='approved') {
      ...
      const all=(await tx.list('returns',{where:[['orderId','==',order.id]]})).filter(r=>r.orderId===order.id&&['pending','approved'].includes(r.status));
      ...
      await refreshInvoiceAmounts(tx,order,context,{payments:await tx.list('payments',{where:[['storeId','==',store.id]]}),returns:all.map(returned=>returned.id===result.id?result:returned)});
    ...
    const builderSize=payload.builderSize??previous?.builderSize??'small';if(!['small','medium','large'].includes(builderSize))fail('INVALID_INPUT','Choose small, medium or large builder items.');
    ...
    result=versioned(previous,{id:actor.uid,uid:actor.uid,theme,builderSize,favorites,notificationPreferences:{inApp:bool(notificationPreferences.inApp,'In-app notifications',true),email:emailEnabled,emailEnabledAt},templates:safeTemplates},now,actor);await tx.set('preferences',actor.uid,result);
    ...
    const target=money(payload.openingBalanceCents,'Reconciled balance',{min:-MONEY_LIMIT}),reason=text(payload.reason,'Reconciliation reason',4000,true),current=storeBalance(await tx.list('ledger',{where:[['storeId','==',store.id]]}),store.id);
...
module.exports={AppError,executeCommand,authorizeStore,calculateOrder,loadProductCategories,inventoryId,storeBalance,MONEY_LIMIT};
