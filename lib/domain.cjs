...
const COMMANDS = new Set(['product.save','category.save','store.save','order.save','order.submit','order.transition','payment.report','payment.verify','inventory.adjust','return.create','return.approve','notification.read','preferences.save','migration.reconcile']);
...
async function executeCommand(tx,actor,command,{now=Date.now(),id=randomUUID}={}) {
  ...
  if(['payment.verify','inventory.adjust','return.approve'].includes(command.type))requireStaff(actor);
  ...
  const context={now,id,actor};let result;
  if(command.type==='category.save') {
    ...
    if(previous.status!=='draft') {
      ...
      const calculated=calculateOrder(previous.lines,products,store),balance=storeBalance(await tx.list('ledger'),store.id);
    ...
    const paymentId=id();result=versioned(null,{id:paymentId,storeId,amountCents:money(payload.amountCents,'Payment amount',{min:1}),method:text(payload.method,'Payment method',100)||'unspecified',reference:text(payload.reference,'Payment reference',500),note:text(payload.note,'Payment note',2000),status:'pending',reportedBy:actor.uid,reportedAt:now},now,actor);
    ...
    if(previous.status==='verified')result=previous;
      ...
      result=versioned(previous,{status:'verified',verifiedBy:actor.uid,verifiedAt:now},now,actor);await tx.set('payments',previous.id,result);
    ...
    const returnId=id();result=versioned(null,{id:returnId,orderId:order.id,storeId:order.storeId,invoiceNumber:order.invoiceNumber,lines,reason:text(payload.reason,'Return reason',2000,true),subtotalCents:money(lines.reduce((s,l)=>s+l.subtotalCents,0)),taxCents:money(lines.reduce((s,l)=>s+l.taxCents,0)),totalCents:money(lines.reduce((s,l)=>s+l.totalCents,0)),status:'pending'},now,actor);
    ...
    result=versioned(previous,{id:actor.uid,uid:actor.uid,theme,favorites,notificationPreferences:{inApp:bool(notificationPreferences.inApp,'In-app notifications',true),email:bool(notificationPreferences.email,'Email notifications',false)},templates:safeTemplates},now,actor);await tx.set('preferences',actor.uid,result);
  ...
  const auditId=id();
  await tx.set('audit',auditId,{id:auditId,commandId,type:command.type,actorUid:actor.uid,actorRole:actor.role,storeId:result?.storeId||(command.type==='store.save'||command.type==='migration.reconcile'?result?.id:null)||null,recordId:result?.id||null,at:now,createdAt:now,version:result?.version||null});
