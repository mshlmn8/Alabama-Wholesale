...
const LIMITS = Object.freeze({ textChars: 12000, imageBytes: 5 * 1024 * 1024, responseBytes: 192 * 1024, catalogBytes: 750 * 1024, products: 2000, variants: 200, quantity: 1000000 });
...
function validateProposal(proposal, products) {
  ...
  if (!object(proposal) || !onlyKeys(proposal, ['lines', 'ambiguities', 'summary']) || !Array.isArray(proposal.lines) || !Array.isArray(proposal.ambiguities) || proposal.ambiguities.length > 100 || !cleanString(proposal.summary, 2000)) invalid();
  ...
  for (let index = 0; index < proposal.lines.length; index++) {
    ...
    else if (!['each', 'case'].includes(line.unit)) reason = 'The requested quantity format could not be verified.';
  ...
  if (!lines.length && !uniqueAmbiguities.length) uniqueAmbiguities.push('Clarify the products, variants, and quantities for the order.');
...
function createProposer({ fetchImpl = globalThis.fetch, timeoutMs = 45000 } = {}) {
  ...
  return async function propose(input, context = {}) {
    ...
    const parts = [...(request.image ? [{ inlineData: request.image }] : []), { text: JSON.stringify({ catalog, request: request.text || 'Read the order photo. Flag unclear text, quantities, or variants for review.' }) }];
