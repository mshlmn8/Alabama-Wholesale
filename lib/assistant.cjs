...
function normalizeCatalog(products) {
  ...
  const catalog = active.map(product => {
    ...
    return {
      id: product.id, name: product.name, variants: [...variants],
...
function validateProposal(proposal, products) {
  ...
  for (let index = 0; index < proposal.lines.length; index++) {
    ...
    else if ((product.variants?.length ? !product.variants.includes(line.variant) : line.variant !== '')) reason = 'The requested variant is not an exact catalog match.';
