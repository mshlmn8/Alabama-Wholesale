...
function prepare(record,store,kind) {
  ...
  if(!Array.isArray(record.lines))issue('INVALID_DOCUMENT','The document does not have valid line items.');
...
async function renderDocument(record,store,kind) {
  ...
  const {orderName}=await import('../public/order-names.mjs');
  return new Promise((resolve,reject)=>{
    const pdf=new PDFDocument({size:'LETTER',margin:42,bufferPages:true,compress:true,info:{Title:`${orderName(record,store,kind).replace(/ — /g,' - ')} - ${model.title}`,Author:'Alabama Wholesale',Subject:'Wholesale order document',Creator:'Alabama Wholesale'}});
