  ...
  return async function find(product, { sourcePage: suppliedPage = "" } = {}) {
    ...
    if (!urls.length) {
      const suggestion = await ask(
        `Suggest up to 3 KNOWN official manufacturer or established supplier product-page URLs for this exact product family. Use your existing knowledge only; no search tool. Do not return search results, shopping search URLs, marketplace listings, or invented URLs. If the brand/product cannot be reliably identified, return an empty array. These URLs will be fetched and checked independently. JSON {"sourcePages":["https://..."],"note":"short uncertainty explanation"}. Product: ${JSON.stringify(identity)}`,
      );
    ...
    for (const candidate of candidates
      ...
      note = plain(verdict.reason, 350) || note;
    }
