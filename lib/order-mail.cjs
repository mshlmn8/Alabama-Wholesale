...
async function deliverOrderMail(
  ...
  async function processCandidate(candidate) {
    ...
    let bytes;
    try {
      // Bounded preparation also protects an abandoned rendering worker's lease.
      bytes = await boundedSend(
        { sendMail: () => render(claimed.order, claimed.store, "invoice") },
        null,
        45000,
      );
      if (
      ...
      const saved = await saveOutcome(
        ...
        claimed.row,
        {
          ...
          lastError: {
            ...
            message:
              "The invoice PDF could not be prepared. The order remains saved online; retry the email after the document issue is fixed.",
          },
        },
        now,
      );
    ...
    const invoice = String(claimed.order.invoiceNumber)
      .replace(/[\r\n]/g, " ")
      .slice(0, 120);
    const store = String(
      claimed.order.storeSnapshot?.name || claimed.order.storeName || "Store",
    )
      .replace(/[\r\n]/g, " ")
      .slice(0, 300);
    let outcome;
    try {
      const info = await boundedSend(
        transport,
        {
          from: sender,
          to: RECIPIENT,
          subject: `Alabama Wholesale order ${invoice} — ${store}`,
          text: `Submitted order ${invoice}\nStore: ${store}\n\nThe confirmed invoice is attached as a PDF.\n\nAlabama Wholesale`,
          messageId,
          attachments: [
            {
              filename: `${invoice.replace(/[^A-Za-z0-9._-]/g, "_")}.pdf`,
              content: bytes,
